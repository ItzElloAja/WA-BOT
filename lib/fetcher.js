...
exports.getBuffer = getBuffer = async (url) => {
    ...
    const anu = fs.readFileSync('./image/Wahyuu.jpg')
...
exports.modStick = (media, client, mek, from) => {
    ...
    try {
        ...
        .on('exit', () => {
            dhenxs.sendMessage(from, fs.readFileSync(out), 'stickerMessage', {quoted: mek})
            fs.unlinkSync(out)
            fs.unlinkSync(media)
        })
    } catch (e) {
        console.log(e)
        dhenxs.sendMessage(from, 'Terjadi keslahan', 'conversation', { quoted: mek })
        fs.unlinkSync(media)
    }
}
